...
static char* file_name = "/data/out.yuv";
...
static void open_device() {
    ...
    #if 0
	ffd = open(file_name, O_WRONLY | O_APPEND);
	if (-1 == ffd) {
...
static void init_mmap() {
	...
	if (req.count < 2) {
		...
		LOGE("Insufficient buffer memory on %s\n", dev_name);
		exit(EXIT_FAILURE);
	}
...
static void init_device() {
	...
	if (ioctl(fd, VIDIOC_DBG_G_CHIP_IDENT, &chip)) {
        	printf("VIDIOC_DBG_G_CHIP_IDENT failed.\n");
       		LOGE("VIDIOC_DBG_G_CHIP_IDENT failed.\n");
        return ;
    }
	printf("TV decoder chip is %s\n", chip.match.name);
	int a = 1;
	  if (xioctl(fd, VIDIOC_S_INPUT, &a) < 0) {
	...
	v4l2_std_id                id;
	if (xioctl(fd, VIDIOC_G_STD, &id) < 0) {
        printf("VIDIOC_G_STD failed\n");
        LOGE("VIDIOC_G_STD failed\n");
        return ;
    }

	if (xioctl(fd, VIDIOC_S_STD, &id) < 0) {
        printf("VIDIOC_S_STD failed\n");
        LOGE("VIDIOC_S_STD failed\n");
        return ;
    }
    ...
    if (xioctl(fd, VIDIOC_G_PARM, &parm) == -1) {
        printf("VIDIOC_G_PARM ioctl error");
        LOGE("VIDIOC_G_PARM ioctl error");
        return ;
    }
...
char video_capture(unsigned char* dst){
	...
	if(FD_ISSET(fd, &fds)){
		...
		memcpy(dst, im_from_cam, IM_WIDTH*IM_HEIGHT*1.5);
        //parse_im(im_from_cam, dst);
	       // int rz = write(ffd, buffers[buf_in_while_loop.index].start, 720*480*1.5);
